...
export default function Slides({ story }: { story: StoryProps }) {
  ...
  const slideMove = (direction: "up" | "down") => {
    if (
      ...
      (position === story.slides.length && direction === "down")
    )
      return;
    setTimeout(() => {
  ...
  return (
    <>
      {/* Invisible Controls */}
      <div
        className="absolute top-0 w-full h-1/3 z-40"
        onClick={() => slideMove("up")}
      ></div>
      <div
        className="absolute bottom-0 w-full h-2/3 z-40"
        onClick={() => slideMove("down")}
      ></div>
        ...
        {story.slides.map((slide, index) => (
          <div key={index}>
            <div className="h-[75dvh] py-24 relative flex flex-col justify-center items-center gap-y-8">
              {/* Date */}
              {slide.date && (
                <span className="btn border-petroleum-900">{slide.date}</span>
              )}
              ...
              {/* Text */}
              <div
                className="px-16 md:px-32 text-center md:text-lg lg:text-2xl"
