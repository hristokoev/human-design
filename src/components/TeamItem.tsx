    ...
    <div>
              ...
              >
                <Dialog.Panel className="w-full max-w-5xl transform overflow-hidden rounded-3xl bg-petroleum-950 shadow-2xl shadow-petroleum-900/50 cursor-pointer">
                  ...
                  <img
                    src={ImageDna.src}
                    alt="DNA Image"
                    className="absolute bottom-0 right-0 h-2/3"
                    aria-hidden={true}
                  />
      ...
      <div className="flex flex-col sm:flex-row gap-8 lg:gap-32">
        ...
        <div className="sm:w-2/3 flex flex-col gap-y-4 sm:gap-y-12">
          ...
          <div className="flex flex-col gap-y-2">
            <div className="self-start">
              {/* Name */}
              <h2 className="text-petroleum-900 text-6xl font-bold">
                {item.name}
              </h2>
            </div>
          ...
          <div className="relative">
            <svg
              className="absolute bottom-0 left-0 w-4 h-4 md:w-6 md:h-6 text-gold-400"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 18 14"
            ...
            <div
              className="w-[90%] mx-auto p-6 lg:p-12 text-xl"
              dangerouslySetInnerHTML={{ __html: item.quote }}
            />
            <svg
              className="absolute top-0 right-0 w-4 h-4 md:w-6 md:h-6 rotate-180 text-gold-400"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 18 14"
          ...
          {item.story?.length && (
            <div className="hidden lg:block mx-auto">
              <button
                onClick={openModal}
                className="btn-lg text-white font-extrabold text-xl shadow-xl shadow-gold-900/10 hover:scale-105 hover:shadow-2xl hover:shadow-gold-900/10 rounded-full bg-[linear-gradient(to_right,theme(colors.gold.200),theme(colors.gold.400),theme(colors.gold.400),theme(colors.gold.200))] bg-[length:200%_auto] hover:animate-gradient"
              >
                Můj příběh
              </button>
            </div>
        ...
        <div className="relative">
          ...
          <img
            src={
              "https://humandesign-admin.occamy.cz/storage/uploads" +
              item.photo.path
            }
            alt={item.name}
          ...
          {item.award?.path && (
            <div className="hidden lg:block absolute bottom-0 left-0 -translate-x-1/2 translate-y-1/4 z-10">
              <img
                ...
                alt="Obrázek ocenění"
                className="w-[16rem] h-[12.8rem] object-contain"
              />
            </div>
          )}
          ...
          {item.award?.path && (
            <div
              className="lg:hidden absolute bottom-0 left-0 w-full flex justify-center z-10"
              aria-hidden={true}
            >
              <img
                src={
                ...
                alt="Obrázek ocenění"
                className="max-w-40 translate-y-12 object-contain"
              />
            </div>
          )}
        </div>
      ...
      <div
        className="mt-16 lg:hidden flex flex-col justify-center gap-y-8"
        aria-hidden={true}
      >
        {/* Counters */}
        ...
        {item.story?.length && (
          <div className="mx-auto">
            <button
              onClick={openModal}
              className="btn-lg text-white font-extrabold text-xl shadow-xl shadow-gold-900/10 rounded-full bg-[linear-gradient(to_right,theme(colors.gold.200),theme(colors.gold.400),theme(colors.gold.400),theme(colors.gold.200))] bg-[length:200%_auto]"
            >
              Můj příběh
            </button>
          </div>
        ...
        {item.certificates?.map((certificate, k) => (
          ...
          >
            <img
              ...
              alt="Obrázek certifikátu"
              className="min-h-40 max-w-52 object-contain"
            />
            <h3 className="w-2/3 text-xl text-gold-400 font-bold uppercase">
              {certificate.title}
            </h3>
            <div
              className="text-sm"
              dangerouslySetInnerHTML={{ __html: certificate.text }}
            />
          </div>
        ))}
      </div>
