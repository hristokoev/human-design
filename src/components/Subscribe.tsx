...
export default function Subsctibe() {
	...
	const handleSubmitForm: SubmitHandler<any> = async (data) => {
		...
		await fetch("/api/subscribe", {
			...
			if (res.status === 200) {
				setFormMessage({ header: "Děkujeme za odběr!", text: "Váš email byl úspěšně přidán k odběru." });
				setFormStatus("success");
			} else {
				setFormMessage({ header: "Něco se pokazilo při odběru!", text: "Zkuste to prosím znovu." });
				setFormStatus("error");
			}
		}).then(() => {
			setLoading(false);
			...
			<div className={`p-4 ${formSubmitted && "bg-petroleum-900"} rounded-md`}>
				{!formSubmitted ? (
						...
						<button className={`flex px-4 py-2 border ${!loading ? 'border-white hover:bg-white hover:text-petroleum-900' : 'cursor-not-allowed'} transition-all duration-150 ease-in-out`} type="submit" disabled={loading}>
							{loading && (
								<div role="status" className="mr-3">
									<svg aria-hidden="true" className="w-5 h-6 text-gray-200 animate-spin fill-petroleum-300" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
					...
					<div className="flex justify-between text-white">
						...
						{formStatus === "error" && (
							<button className="px-4 py-2 border border-white hover:bg-white hover:text-petroleum-900 transition-all duration-150 ease-in-out" onClick={() => setFormSubmitted(false)}>Zkusit znovu</button>
						)}
					</div>
				)}
			</div>
